    ...
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <form method="POST" action="{{ route('admin.content.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            @foreach ($groupedSchema as $group => $fields)
                <section class="mk-card p-6">
                    <div class="mb-4 flex flex-col gap-1">
                        <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-50">{{ $groupLabels[$group] ?? ucfirst($group) }}</h2>
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Ini halaman lama untuk konten campuran teks + media.') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        @foreach ($fields as $field => $meta)
                            ...
                            <div class="{{ $fieldType === 'textarea' ? 'md:col-span-2' : '' }}">
                                <label for="{{ $field }}" class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ $meta['label'] }}</label>
                                ...
                                @elseif ($fieldType === 'image')
                                    <div class="mt-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60">
                                        @if ($imageUrl)
                                            <img src="{{ $imageUrl }}" alt="{{ $altValue ?: $meta['label'] }}" class="h-32 w-full rounded-xl border border-slate-200 object-cover bg-white dark:border-slate-800 dark:bg-slate-950">
                                        @else
                                            <div class="flex h-32 items-center justify-center rounded-xl border border-slate-200 bg-white text-xs text-slate-400 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-500">
                                                {{ __('Belum ada gambar') }}
                                            </div>
                                        @endif
                                        ...
                                        <div class="mt-3 space-y-3">
                                            <input
                                                ...
                                                type="file"
                                                accept="image/*"
                                                class="input w-full rounded-xl px-3 py-2 text-xs text-slate-600 dark:text-slate-300"
                                            >
                                            <input
                                                name="{{ $field }}_alt"
                                                type="text"
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Untuk mengedit teks situs pengguna, gunakan menu baru:') }} <strong>{{ __('Teks Website') }}</strong>.</p>
