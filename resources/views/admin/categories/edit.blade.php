...
@section('title', __('ui.admin_categories.form.edit_title'))
@section('page_title', __('ui.admin_categories.form.edit_page_title'))
    ...
    <div class="admin-categories-page mx-auto max-w-4xl space-y-5 sm:space-y-6">
        <section class="admin-categories-card p-5 sm:p-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="admin-categories-kicker">{{ $formCopy['edit_kicker'] }}</p>
                    <h2 class="admin-categories-title mt-2 text-2xl font-black">{{ $formCopy['edit_heading'] }}</h2>
                    <p class="admin-categories-muted mt-1 text-sm">{{ $formCopy['edit_subtitle'] }}</p>
                </div>
                <a
            ...
            <form method="POST" action="{{ route('admin.categories.update', $category->slug) }}" class="mt-6 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <p class="text-xs admin-categories-muted">{{ $formCopy['edit_note'] }}</p>
