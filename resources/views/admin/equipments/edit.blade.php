    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <section class="mk-card p-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-blue-700 dark:text-blue-400">{{ __('Ubah Alat') }}</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Perbarui detail, slug, dan status alat.') }}</p>
                </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/60">
                    <p class="text-[11px] uppercase tracking-widest text-slate-500 dark:text-slate-400">{{ __('Total Stok') }}</p>
                    <p class="mt-1 text-xl font-semibold text-slate-900 dark:text-slate-50">{{ $equipment->stock }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/60">
                    <p class="text-[11px] uppercase tracking-widest text-slate-500 dark:text-slate-400">{{ __('Sedang Dipakai') }}</p>
                    <p class="mt-1 text-xl font-semibold text-amber-600">{{ $equipment->reserved_units }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/60">
                    <p class="text-[11px] uppercase tracking-widest text-slate-500 dark:text-slate-400">{{ __('Unit Tersedia') }}</p>
            ...
            <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-50">{{ __('Kalender Pemakaian Alat') }}</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Status alat tetap ready, tapi bentrok tanggal sewa akan ditolak saat checkout.') }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <a
                            href="{{ route('admin.equipments.edit', ['slug' => $equipment->slug, 'month' => $bookingCalendar['previous_month']]) }}"
                            class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:border-blue-200 hover:text-blue-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300"
                        >
                            ←
                        </a>
                        <span class="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 dark:bg-slate-950 dark:text-slate-300">{{ $bookingCalendar['month_label'] }}</span>
                        <a
                ...
                <div class="mt-3 -mx-1 overflow-x-auto px-1 pb-1 sm:mx-0 sm:overflow-visible sm:px-0">
                    <div class="min-w-[640px]">
                        <div class="grid grid-cols-7 gap-2 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            <span>{{ __('Sen') }}</span>
                            <span>{{ __('Sel') }}</span>
                            <span>{{ __('Rab') }}</span>
                            <span>{{ __('Kam') }}</span>
                        ...
                        <div class="mt-2 grid grid-cols-7 gap-2">
                            @foreach ($bookingCalendar['days'] as $day)
                                <div class="min-h-[68px] rounded-lg border px-2 py-2 {{ $day['in_month'] ? 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950' : 'border-slate-100 bg-slate-100 text-slate-400 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-500' }}">
                                    <p class="text-xs font-semibold dark:text-slate-200">{{ $day['day'] }}</p>
                                    @if ($day['booked_qty'] > 0)
                                        <p class="mt-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
                                            {{ __('Disewa') }} {{ $day['booked_qty'] }}
                                        </p>
                                    @else
                                        <p class="mt-1 text-[10px] text-slate-400 dark:text-slate-500">-</p>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="mt-3 space-y-2">
                    @forelse ($bookingCalendar['events'] as $event)
                        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400">
                            <p class="font-semibold text-slate-800 dark:text-slate-100">{{ $event['order_number'] }} • {{ $event['customer'] }}</p>
                    ...
                    @empty
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Belum ada jadwal sewa pada bulan ini.') }}</p>
                    @endforelse
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="input mt-2 w-full rounded-xl px-3 py-2 text-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-500/30 focus:outline-none"
                        >
                        <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">{{ __('Unit tersedia dihitung: total stok - unit yang sedang dipakai.') }}</p>
                    </div>
                    <div>
