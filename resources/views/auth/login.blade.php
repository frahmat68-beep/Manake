...
>
    ...
    @if (session('error') || session('status') || $errors->any())
        <div class="mb-2 flex w-full flex-col gap-2">
            @if (session('error'))
                <div class="text-center text-sm text-red-400">{{ session('error') }}</div>
            @endif
            @if (session('status'))
                <div class="text-center text-sm text-emerald-400">{{ session('status') }}</div>
            @endif
            @if ($errors->any())
    ...
    <form method="POST" action="{{ route('login') }}" class="w-full flex flex-col gap-4">
        ...
        <div class="mt-2 w-full text-center">
            <span class="text-xs text-gray-400 dark:text-gray-500">
                ...
                <a href="{{ route('register') }}" class="font-medium text-blue-500 transition hover:text-blue-400" data-skip-loader="true">
