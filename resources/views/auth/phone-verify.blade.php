...
@section('title', 'Verifikasi Nomor Telepon')
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
                ...
                <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <form method="POST" action="{{ route('phone.otp.verify') }}" class="space-y-3 rounded-xl border border-slate-200 p-4">
                        @csrf
                        <h2 class="text-sm font-semibold text-slate-900">2) Verifikasi OTP</h2>
                        <label class="block text-xs font-semibold text-slate-500">Kode OTP</label>
                        <input
                            type="text"
                            name="otp"
                            maxlength="6"
                ...
                <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
                    ...
                    @if ($profile?->phone_verified_at)
                        <span class="font-semibold text-emerald-700">Terverifikasi ({{ $profile->phone_verified_at->format('d M Y H:i') }})</span>
                    @else
